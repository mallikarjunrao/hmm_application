class Ownify::StaticPagesController < LoginController
  def index
  ...
  def list
    if get_access_level != 1
      ...
      @dyno_type=DynoType.find_by_slug('about')
  ...
  def create
    ...
    @dyno_page.page_on= Time.now
    # SEO stuff
    seo = SeoSetting.new
    seo.path = "/#{@dyno_page.slug}"
    seo.page_title = "Crave Cupcakes - #{@dyno_page.title}"
    seo.meta_description = @dyno_page.title
    seo.meta_keywords = @dyno_page.title
    seo.save
    if @dyno_page.save
      expire_dyno_pages
  ...
  def seo
    @dyno_page = DynoPage.find(params[:id])
    @seo_setting = SeoSetting.find_by_path( "/#{@dyno_page.slug}" )
  end
