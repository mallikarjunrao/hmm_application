class Cupcake < ActiveRecord::Base
  has_many :cupcakes_day, :dependent => true
  has_many :days, :through => :cupcakes_day
  ...
  def day_ids=(day_ids)
    cupcakes_day.each do |cup_day|
      cup_day.destroy unless day_ids.include? cup_day.day_id
    end
				
    day_ids.each do |day_id|
      self.cupcakes_day.create(:day_id => day_id) unless cupcakes_day.any? { |d| d.day_id == day_id }
    end
  end
