package com.dougmccune.containers
{
	...
	public class CarouselContainer extends BasePV3DContainer
	{
		...
		[Bindable]
		public var componentSpacing : int = 110;
		...
		private function moveCamera():void {
			/*  camera.x =  Math.cos(_angle) *(width); 
   		    ...
   		    camera.x =  Math.cos(_angle) *(rad+10); 
   			camera.z = Math.sin(_angle) *(rad+10);
		}
		...
		override protected function layoutChildren(unscaledWidth:Number, unscaledHeight:Number):void {
			...
			var radius:Number = ((componentSpacing + childWidth) * numOfItems)/(Math.PI*2);
			rad = radius;
			...
			camera.zoom = 1 + 20/(radius);
			camera.focus = radius/2;
