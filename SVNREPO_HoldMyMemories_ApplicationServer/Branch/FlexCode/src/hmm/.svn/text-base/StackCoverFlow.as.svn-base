package hmm
{
	...
	[Event("change")]
	public class StackCoverFlow extends EventDispatcher
	{
		...
		private var planeBitmapDictionary : Dictionary = new Dictionary();
		...
		public function StackCoverFlow(pv3dSprite:Sprite, bitmaps : Array, xoff: Number, yoff: Number, zoff: Number, direction : Boolean)
		{
		...
		private function setupScene():void
		{
		  ...
		  camera.extra.goPosition.copyPosition( camera );
			...
			for(var i : int = 0; i < bitmapAssets.length; i++)
			{
				...
				var plane : Plane = new Plane(bitmap,0, 0, 3,3);
				...
				refPlane.y = plane.x - plane.segmentsH*3 -20;
				refPlane.scaleY = 1;
				refPlane.rotationY = -90;
				if(labelField)
				{
			...
			if(animationPlanes[0])
			{
				...
				animationPlanes[0].extra.goto.rotationY = -80;	
		...
		private function doRollOver(event : Event) : void
		{
			...
			//plane.extra.goto.scaleX = 1.1;
			//plane.extra.goto.scaleY = 1.1;
			//plane.extra.goto.scaleZ = 1.1;
		...
		private function doRollOut(event : Event) : void
		{
			...
			//plane.extra.goto.scaleX = 1.0;
			//plane.extra.goto.scaleY = 1;
		...
		{
			if(selectedPlane == bitmapPlanes[ event.target ])
			{
				var evt : CoverFlowEvent = new CoverFlowEvent(CoverFlowEvent.NAVIGATE_TO);
				evt.extra = assetPlaneDictionary[event.target];
				dispatchEvent(evt);
			...
			selectedPlane.extra.goto.rotationY = -80;
			var evt : CoverFlowEvent = new CoverFlowEvent("change");
		...
		private function onEnterFrame( event :Event ):void
		{
			...
			for(var i : int= 0; i < animationPlanes.length; i++)
			{
				...
				if(goto.x == 300 && Math.abs((bPlane.x - goto.x) / 32) < 3)
				{
					...
					bPlane.extra.goto.rotationY = -90;
				}
