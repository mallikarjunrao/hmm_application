package components
{
	...
	*/
	public class DisplayStack extends UIComponent implements IHistoryManagerClient, IFocusManagerComponent
	{
		//---------------------------------------------------------------------------------------
		// constants
		//---------------------------------------------------------------------------------------
		...
		private const kPaneOverlap:Number = 175;
		...
		private var lastCP : ChildPosition= new ChildPosition();
		...
		*/
		...
		public function set selectedIndex(value:Number):void
		{
			...
			for(var i : int = _selectedIndex; i < _children.length; i++)
			{
				var t : TiltingPane = _children[i];
				var scale : Number = Math.max(0,(1 - 0.2*(i-_selectedIndex) ));
				trace("SelectedIndex: "+_selectedIndex+" index: " + i+ " CurrentScale: "+scale+ " PrevScale: "+t.scaleX);
				//t.scaleParams(scale, scale);
			}
		...
		*/
		...
		{
			var delta : Number = value - _currentPosition;
			trace(delta.toString());
			_currentPosition = value;
			for(var i = 0; i < _children.length; i++)
			{
				var t : TiltingPane = _children[i];
				//t.scaleX = Math.min(1, t.scaleX+ delta);
				//t.scaleY = Math.min(1, t.scaleY+ delta);
			}
			invalidateDisplayList();
		}
		...
		override protected function updateDisplayList(unscaledWidth:Number, unscaledHeight:Number):void
		{
			...
			*/
			for(var i:int = 0;i<_children.length;i++)
			{
				...
				*/
				//trace("the scale value for index "+i+" is :"+ c.scale);
			...
			*/
			t = _children[Math.round(_currentPosition)];
			t.alpha = 1;
			setChildIndex(t,numChildren-1);
			
				/* var m : Matrix = new Matrix();
				m.createGradientBox(unscaledWidth, unscaledHeight, Math.PI/2);
				var g : Graphics = graphics;
				g.beginGradientFill(GradientType.LINEAR, [ 0x0f0f0f, 0x1f1f1f, 0x4f4f4f], [0.5, 0.4, 0.4], [ 150, 220, 255],m);
				g.moveTo(unscaledWidth/10, unscaledHeight*2/3);
				g.lineTo(0, unscaledHeight);
				g.lineTo(unscaledWidth, unscaledHeight);
				g.lineTo(unscaledWidth*9/10, unscaledHeight*2/3);
				g.lineTo(unscaledWidth/10, unscaledHeight*2/3);
				g.endFill(); */
		...
		*/
		private function calcPositionForSelection(i:Number,sel:Number,c:ChildPosition):void
		{
			...
			*/
			...
			var val : Number;
			/* and compute a weighted averagei
			*/
			c.angle = 0;// lCP.angle + delta * (rCP.angle - lCP.angle);
			if((rCP.scale - lCP.scale) == 0 && i != 0)
			{
				var cur : Number = Math.max(0,(1 - 0.2*i ));
				var prev : Number = Math.max(0,(1 - 0.2*(i-1)) );
				var scale : Number = prev - cur;
				val = scale*delta;	
			}
			 else
			 	val = delta*(rCP.scale - lCP.scale);
			
			/* if(val == 0)
			{
				val = -(1 -  1/(i+1));
			} */
			c.scale = lCP.scale +val ;
			//trace("index: "+i+" value: "+ val);
			c.x = lCP.x + delta * (rCP.x - lCP.x);
			//unscaledHeight/2 - t.getExplicitOrMeasuredHeight()* (c.scale)/2
			//c.y = lCP.y + delta * (rCP.y - lCP.y);
			c.y = (unscaledHeight - t.getExplicitOrMeasuredHeight()*c.scale)/2;
		}
		...
		*/
		private function calcPositionForIndexSelection(i:int,sel:Number,c:ChildPosition):void
		{			
			var t:TiltingPane = _children[i];
			...
			if(i == sel)
			{
				...
				*/
				...
				c.x = t.getExplicitOrMeasuredWidth()/2;
			...
			else if (i < sel)
			{
				...
				*/
				...
				var rightBase:Number  =  0- selected.widthForAngle(0)/2 + (adjacent.widthForAngle(-_angle)*3/10 - adjacent.getExplicitOrMeasuredWidth()/2) * c.scale*1.3;
				...
				*/
				c.x = -rightBase - kPaneOverlap*(_children.length - sel + i+2);
				/*	lastly, center it vertically */
				c.y = unscaledHeight/2 - t.getExplicitOrMeasuredHeight()* (c.scale)/2;
			}
			else
			{
				// back
				/* 	this is basically the same logic as above, but for children to the right of the selection.  Iot sets it to 
				*	a negative angle, and calculates the position as a distance from the first child to the right of the selection.
				*/
				c.scale = Math.max(0, (1- 0.2*i));
				adjacent = _children[sel + 1];
				rightBase =  adjacent.widthForAngle(0) + (adjacent.widthForAngle(-_angle)*8/10 - adjacent.getExplicitOrMeasuredWidth()/2) * c.scale*1.3;
				c.angle = 0;
		...
		*/
		private function itemClickHandler(e:MouseEvent):void
		{
			...
			*/
			...
			var index:Number = _children.indexOf(e.currentTarget);
			if(index > -1)
				selectedIndex = index;
		}
		...
		*/
	    private function startAnimation():void
	    {
			...
			*/
			_animation.duration = Math.max(900,Math.abs(_selectedIndex - _currentPosition) * 1200);
			_animation.easingFunction = mx.effects.easing.Quadratic.easeOut;
			_animation.addEventListener(EffectEvent.EFFECT_END, handleAnimationEnd);
			_animation.play();
			//_fade.target = _children[_selectedIndex - 1];
			//_fade.play();
			
		}
		
		private function handleAnimationEnd(event : Event) : void
		{
			for( var i : int = 0; i < _currentPosition; i++)
			{
				var obj : Object = _children.shift();
				_children.push(obj);
			}
			var c:ChildPosition = new ChildPosition();
			var t:TiltingPane;
