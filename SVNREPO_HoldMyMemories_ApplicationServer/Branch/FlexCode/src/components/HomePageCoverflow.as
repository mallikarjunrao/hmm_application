package components
{
	...
	[Event("change")]
	public class HomePageCoverflow extends EventDispatcher
	{
		protected var camera:Camera3D;
		protected var scene:Scene3D;
		private var rootNode:DisplayObject3D;
		protected var pv3dSprite:Sprite;
		protected var bitmapPlanes : Dictionary;
		private var keyRight:Boolean = false;
		private var keyLeft:Boolean = false;
		private var keyForward:Boolean = false;
		private var keyReverse:Boolean = false;
		protected var lastImagePosition : Number3D = new Number3D();
		protected var firstImagePosition : Number3D = new Number3D();
		public var  bitmapAssets : Array;
		public var  animationPlanes : Array;
		private var zoomed : Boolean = false;
		protected var cameraDefault : DisplayObject3D = new DisplayObject3D();
		protected var renderList : Array = new Array();
		protected var _refPlanes : Dictionary = new Dictionary();
		...
		protected var selectedPlane:Plane;
		protected var selectedIndex : int;
		protected var assetPlaneDictionary : Dictionary = new Dictionary();
		protected var planeBitmapDictionary : Dictionary = new Dictionary();
		...
		public function HomePageCoverflow(pv3dSprite:Sprite, bitmaps : Array, xoff: Number, yoff: Number, zoff: Number, direction : Boolean)
		{
		...
		{
			if(count == renderList.length-1)
				count = 0;
			selectedPlane = renderList[ count ];
			//selectedPlane.scaleX = 1.3;
			//selectedPlane.scaleY = 1;
			
			
			animationPlanes.sortOn(["x"], [Array.NUMERIC| Array.DESCENDING]);
			var idx : int = animationPlanes.indexOf(selectedPlane);
			...
			selectedPlane.extra.goto.rotationY = -90;
			count++;
		}
		...
		protected function setupScene():void
		{
		  ...
		  camera.extra.goPosition.copyPosition( camera );
			...
			if(animationPlanes[0])
			{
				...
				animationPlanes[0].extra.goto.rotationY = -90;
				selectedPlane = animationPlanes[0];	
		...
		protected function handleFileLoaded(event : FileLoadEvent) : void
		{
			...
			var bitmap : BitmapFileMaterial = event.target as  BitmapFileMaterial;
			var plane : Plane = planeBitmapDictionary[event.target];
			var originalHeight : Number = bitmap.maxV*bitmap.bitmap.height; 
			var originalWidth : Number = bitmap.maxU*bitmap.bitmap.width;
			var ratio : Number = (originalHeight/originalWidth);
			var refPlane : Plane = _refPlanes[plane];
			if(originalHeight < originalWidth)
			{
				plane.scaleX = 0.7;
				plane.scaleY = ratio*0.7;	
			}else
			{
				plane.scaleX = 0.5;
				plane.scaleY = 0.5*ratio;
			}
			
			refPlane.scaleX = plane.scaleX;
			refPlane.scaleY = plane.scaleY;
			var gotoData : DisplayObject3D = plane.extra.goto as DisplayObject3D;
			if(gotoData)
			{
				gotoData.scaleX = plane.scaleX;
				gotoData.scaleY = plane.scaleY;
				gotoData.scaleZ = 1;
			}
			//(planeBitmapDictionary[event.target] as Plane).material = event.target as BitmapFileMaterial;
		}
		
		protected function doRollOver(event : Event) : void
		{
		...
		protected function doRollOut(event : Event) : void
		{
			var plane:Plane = bitmapPlanes[ event.target ];
			//plane.scaleX = 1.3;
			//plane.scaleY = 1;
		...
		protected function doPress(event : Event) : void
		{
			if(bitmapPlanes[ event.target ] == selectedPlane)
			{
				var evt : CoverFlowEvent = new CoverFlowEvent(CoverFlowEvent.NAVIGATE_TO);
				evt.extra = assetPlaneDictionary[event.target];
				dispatchEvent(evt);
		...
		protected function onEnterFrame( event :Event ):void
		{
			...
			for(var i : int= 0; i < animationPlanes.length; i++)
			{
				...
				if(goto.x == 300 && Math.abs((bPlane.x - goto.x) / 32) < 3)
				{
					...
					bPlane.extra.goto.rotationY = -80;
				}
