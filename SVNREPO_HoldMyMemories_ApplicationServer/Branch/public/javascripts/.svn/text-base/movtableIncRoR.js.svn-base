...
var limit = null; // document.getElementsByName("limit").options[selectedIndex]
...
function sortableInit()
{
	...
	//limit = document.form["movform"].limit.options[selectedIndex].text;
       // limit = document.getElementsByName("limit").options[selectedIndex].text
}
...
function getPopUpMenuObj(e)
{
	...
	parentDoc = (e.target) ? e.target.Document : e.srcElement.Document; 
  ...
  if (targetElm.tagName=='TD' || targetElm.tagName=='TH') {
	...
	filterurl=baseurl+'?filterfield='+myTable.columns.getColByID(colID).fieldName+"&filtervalue="+ selectedText +'&filteroption=Equal'+'&limit='+ limit;
	removeFilter=baseurl
	...
	if (targetElm.tagName=='TH') {
		...
		if (JScolhide==1)
			if ((myTable.columns.item.length-hiddenCols.length)==1) 
				cursorMenu1.addMenuItem(new menuItem(getText("Hide Column"), "", "",false));
			else
				//cursorMenu1.addMenuItem(new menuItem("Nascondi Colonna", "", "code:myTable.columns.item["+getCellIndex(targetElm)+"].hideColumn()"));
				cursorMenu1.addMenuItem(new menuItem(getText("Hide Column"), "", "code:myTable.columns.getColByID('"+colID+"').hideColumn()"));
			
		//Controlla se ci sono colonne nascoste
		if (myTable.columns.hidden()) {
			cursorMenu1.addMenuItem(new menuItem(getText("Show Columns"), "mColonne", "code:myTable.columns.listInvisible()"));
			...
			cursorMenu1.addMenuItem(new menuItem(getText("Show Columns"), "mColonne", "code:myTable.columns.listInvisible()",false));
	...
	if (targetElm.tagName=='TD') {
		...
		{
			cursorMenu1.addMenuItem(new menuItem(getText("Filter by selection"), "", filterurl));
			cursorMenu1.addMenuItem(new menuItem(getText("Filter out selection"), "", filterurl+'&filteroption=Exclude'));
		}
		else
		{
			cursorMenu1.addMenuItem(new menuItem(getText("Filter by selection"), "", "",false));
			cursorMenu1.addMenuItem(new menuItem(getText("Filter out selection"), "", "",false));
		}			
		if (filtered) 
			cursorMenu1.addMenuItem(new menuItem(getText("Remove Filter"), "", removeFilter));
		else
			cursorMenu1.addMenuItem(new menuItem(getText("Remove Filter"), "", removeFilter,false));
		cursorMenu1.addMenuItem(new menuItem("-"));
	}

	//Sort menu items
	if (myTable.columns.getColByID(colID).sortable) {
		cursorMenu1.addMenuItem(new menuItem(getText("Ascending Order"), "", sorturl+'&direction=ASC'));
		cursorMenu1.addMenuItem(new menuItem(getText("Decending Order"), "", sorturl+'&direction=DESC'));
	}
	else
	{	
		cursorMenu1.addMenuItem(new menuItem(getText("Ascending Order"), "", "",false));
		cursorMenu1.addMenuItem(new menuItem(getText("Decending Order"), "", "",false));
  	}
	cursorMenu1.addMenuItem(new menuItem(getText("Copy"), "", "code:copyElement(selectedText)"));
	return cursorMenu1;
  }
  else 
  	return null;
