package com.dougmccune.containers
{
	...
	public class BasePV3DContainer extends ViewStack
	{
		 ...
		 * CPU can't keep up).
		 */
		public var tweenDuration:Number = 1.5;
		...
		protected function containerRollOver(event : MouseEvent) : void
		{
			
		}
		
		protected function containerRollOut(event : MouseEvent) : void
		{
		...
		protected function createPlaneForChild(child:DisplayObject):void {
			...
			//mouse clicks
			plane.container.addEventListener(MouseEvent.CLICK, containerClicked);
			plane.container.addEventListener(MouseEvent.MOUSE_OVER, containerRollOver);
			plane.container.addEventListener(MouseEvent.MOUSE_OUT, containerRollOut);
			objectsToPlanes[child] = plane;
		}
