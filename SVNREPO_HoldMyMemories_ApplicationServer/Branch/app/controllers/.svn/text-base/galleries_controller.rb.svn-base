...
#  end
  ...
  def createGallery
    ...
    gallery.d_gallery_date = Time.now
    gallery.img_url = "http://content.holdmymemories.com"
    gallery.save 
    @result = gallery.id
	render :layout => false
  end
        ...
        when "private":
            ...
            when "private":
               ...
               # sql.execute("update galleries,user_contents set galleries.e_gallery_acess='#{params[:permission]}',user_contents.e_access='#{params[:permission]}'  where galleries.id=#{params[:id]} and user_contents.gallery_id=#{params[:id]}") ;
                  sql.execute("update galleries set galleries.e_gallery_acess='#{params[:permission]}' where galleries.id=#{params[:id]}") ;
                   sql.execute("update user_contents set user_contents.e_access='#{params[:permission]}'  where user_contents.gallery_id=#{params[:id]}") ;
                 sql.commit_db_transaction;
            when "semiprivate":
                #change permission of chapter subchapter gallery and contents
                sql = ActiveRecord::Base.connection();
                ...
                sql.execute("update tags set tags.e_access='#{params[:permission]}' where tags.id in (select sub_chapters.tagid from  sub_chapters, galleries where sub_chapters.id=galleries.subchapter_id and galleries.id=#{params[:id]} )") ;
                sql.execute("update sub_chapters set sub_chapters.e_access='#{params[:permission]}' where  sub_chapters.id in ( select galleries.subchapter_id from galleries whare  galleries.id=#{params[:id]} )") ;
                sql.execute("update galleries set galleries.e_gallery_acess='#{params[:permission]}' where galleries.id=#{params[:id]}") ;
                sql.execute("update user_contents set user_contents.e_access='#{params[:permission]}'  where user_contents.gallery_id=#{params[:id]}") ;
                sql.commit_db_transaction;
                @result = "chapter$subchapter"
  ...
  def setThumbnail
	  ...
	  if(content != nil)
		  if(content.e_filetype == "image")
			  ...
			  folderImage = Magick::Image.read("#{RAILS_ROOT}/public/user_content/photos/icon_thumbnails/videogallery.png").first
  ...
  def gallery_coverflow
    ...
    sort = case params['sort']
           ...
           @sub_chapters_galleries_pages, @sub_chapters_gallerie = paginate :galleriess , :order => sort, :per_page => items_per_page, :conditions => "subchapter_id  =#{params[:id]} and status='active' #{e_access}" ,:order => " #{@sort.to_s} #{@order.to_s} "
  ...
  def showGallery
	  ...
	  @contents = UserContent.find(:all, :conditions => "gallery_id=#{params[:id]} and status='active' #{e_access}",:order =>'id ASC')
	  render :layout => false
  end
  ...
  def showAudioGallery
	  ...
	  @contents = UserContent.find(:all, :conditions => "gallery_id=#{params[:id]} and status='active' #{e_access}",:order =>'id desc')
	  render :layout => false
  end
...
#     end
     ...
     $gal=params[:id]
      ...
      @user_contents_pages, @user_content = paginate :user_contents, :conditions=>"gallery_id=#{params[:id]} && uid=#{logged_in_hmm_user.id}", :order => " #{@sort.to_s} #{@order.to_s} "
            if @params[:order]=='asc'
               @params[:order]='desc'
            else
             if @params[:order]=='desc'
