...
class PaymentUpgradeController < ApplicationController
  ...
  def upgrade_payment
    ...
    email = params[:email]
    if(logged_in_hmm_user.subscriptionnumber.nil?)
      invoicenumber = "HMM000"+"HMM#{params[:hmm_id]}"
    else
      if(logged_in_hmm_user.subscriptionnumber=="One time payment")
        invoicenumber = "HMM000"+"HMM#{params[:hmm_id]}00"+"HMM#{params[:hmm_id]}"
      end
    end  
    ...
    if(flag==1)
      ...
      redirect_to "http://www.holdmymemories.com/customers/upgrade/#{acctype}"
    else
      ...
      #:cvv => cvv_no
        ...
        if response.success?
          ...
          hmm_user_det=HmmUser.find(params[:hmm_id])
              ...
              hmm_user_det[:substatus] = ''
              hmm_user_det[:suspended_date] = ''
              hmm_user_det[:cancel_status] = ''
              ...
              apiresp.messages.each { |message| 
               ...
               @res =  message.text
                ...
                if(@res == "The subscription has already been canceled." || @res == "Successful." || @res == "no subscription") 
                  ...
                  redirect_to "http://www.holdmymemories.com/customers/cancelled_sub/"
                end
