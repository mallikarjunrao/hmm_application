 ...
 def memories
   ...
   sql.update "UPDATE shares SET shown='true' WHERE id = '#{params[:id]}'";
      @swfName = "PhotoGallery"
       @presenter_name=Share.find(:all, :joins => "as a, hmm_users as b", :conditions => "a.id=#{params[:id]} and a.presenter_id=b.id")    
...
#      end
  ...
  def fnfAlert
    ...
    @total_alertcount = (@chapcmt + @subcmt + @galcmt + @momentcmt + @sharemomentcomment + @sharemomentcnt + @share_journal + @total1 + @total + @total2 + @guestcmt + @sharecmt + @msgboardcmt )
  ...
  def subchap_coverflow
   ...
   #        if request.xml_http_request?
    ...
    sort = case params['sort']
           ...
           #@tags_pages, @tags = paginate :tags , :order => sort, :per_page => items_per_page, :joins => 'as b , user_contents as a ', :conditions => "a.uid=#{logged_in_hmm_user.id} and  a.uid=b.uid"
           @sub_chapters_pages, @sub_chapters = paginate :sub_chapters ,  :per_page => items_per_page, :conditions => "tagid=#{params[:id]} #{e_access} and status='active'", :order => " id desc "
 ...
 share.save
 flash[:notice_reg]="The \"Shared Moments\" removed sucessfully!"
