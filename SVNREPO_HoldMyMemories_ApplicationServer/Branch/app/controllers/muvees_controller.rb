  ...
  def test_webser
   ...
   begin
    ...
    for filename in params[:chk]
      ...
      logger.info("sucessful file transfer for file #{filename}")
  ...
  #pp jobData
   begin
      jobid = soap.createMuvee(muveePassport,jobData)
         logger.info("sucessful job processed ")
   rescue
      logger.info("video creation failed")
   end 
   begin
      ftp.close
      logger.info("sucessful ftp server close ")
   rescue
