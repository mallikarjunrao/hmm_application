...
jQuery.fn.daterangepicker = function(settings){
	...
	var datepickerOptions = {
		onSelect: function(dateText, inst) {
				...
				jc(this).trigger('constrainOtherPicker');
...
alert(this);
	...
	function positionRP(){
        ...
        if(options.posX && options.posY){
            var a = jQuery("#textfields");
            if((a.length > 0) && (jQuery(window).width() > 900)){
                    rp.parent().css('left', a.position().left - options.posX).css('top', a.position().left - options.posY);
            }else{
                rp.parent().css('left',a.position().left- options.posX/2).css('top', a.offset().top - options.posY);
            }
        }else{
