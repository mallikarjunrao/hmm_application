...
before_filter :authenticate, :only => [:allcmt_list]#, :edit, :deleteSelection, :deleteSelection1, :update]
  ...
  def create
    ...
    respond_to do |format|
      if @photo_comment.save
        ...
        $notice_pcc
      flash[:notice_pcc] = 'Thank-you for adding your comment.'
 ...
 def create_cmt
    ...
    @photo_comment['v_name']=logged_in_hmm_user.v_fname
    @photo_comment['v_email']=logged_in_hmm_user.v_e_mail
    @photo_comment['v_comment']=params[:photo_comments][:v_comment]
    @photo_comment['uid']=logged_in_hmm_user.id
    
    @photo_comment['d_add_date']=Time.now
    @photo_comment['user_content_id']=params[:orgid]
    @photo_comment['journal_id']=params[:journal_id]
    ...
    @photo_comment.save
     ...
     @commentedTo_usercontent = UserContent.find(params[:user_content_id])
     @commentedTo_details=HmmUser.find(@commentedTo_usercontent.uid)
       
    Postoffice.deliver_usercontentComment(params[:photo_comments][:v_comment],journal,@commentedTo_usercontent.e_filetype,logged_in_hmm_user.v_user_name,logged_in_hmm_user.v_e_mail,@commentedTo_details.v_fname,@commentedTo_details.v_e_mail,logged_in_hmm_user.v_myimage)
...
def new_jcmt
 ...
 def create_jcmt
    ...
    @photo_comment['v_name']=logged_in_hmm_user.v_fname
    @photo_comment['v_email']=logged_in_hmm_user.v_e_mail
    if params[:photo_comments][:v_comment] != ''
       @photo_comment['v_comment']=params[:photo_comments][:v_comment]
           puts "vivek in photo_comment"
    else
       @photo_comment['v_comment']=params[:v_comment]
       puts "vivek"
    end
