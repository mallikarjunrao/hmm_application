class FamilywebsiteContestController < ApplicationController
  ...
  def intro
    ...
    @contest_phase=Base64.decode64(params[:p])
    @contest_phase=@contest_phase.to_i
    @contest_details=ContestDetail.find_by_id(@contest_phase)
    if @contest_phase == 15
      @topentries_photo_0_to_2 = Contest.find(:all,:select => "a.*, b.*, a.id as contest_id, a.first_name as contest_fname" ,:joins=>"as a, user_contents as b",
        :conditions => "a.moment_id = b.id and a.moment_type = 'image' and a.contest_phase='phase#{@contest_phase}' and a.status='active' and age_groups='0-2'", :order => "a.votes desc", :limit =>"2")
      ...
      @topentries_photo_3_to_5 = Contest.find(:all,:select => "a.*, b.*, a.id as contest_id, a.first_name as contest_fname" ,:joins=>"as a, user_contents as b",
        :conditions => "a.moment_id = b.id and a.moment_type = 'image' and a.contest_phase='phase#{@contest_phase}' and a.age_groups='3-5' and a.status='active'", :order => "a.votes desc", :limit =>"2")
      ...
      @topentries_photo_6_to_9 = Contest.find(:all,:select => "a.*, b.*, a.id as contest_id, a.first_name as contest_fname" ,:joins=>"as a, user_contents as b",
        :conditions => "a.moment_id = b.id and a.moment_type = 'image' and a.contest_phase='phase#{@contest_phase}' and a.age_groups='6-9' and a.status='active'", :order => "a.votes desc", :limit =>"2")
      ...
      @topentries_photo_10_to_17 = Contest.find(:all,:select => "a.*, b.*, a.id as contest_id, a.first_name as contest_fname" ,:joins=>"as a, user_contents as b",
        :conditions => "a.moment_id = b.id and a.moment_type = 'image' and a.contest_phase='phase#{@contest_phase}' and a.age_groups='10-17' and a.status='active'", :order => "a.votes desc", :limit =>"2")
      ...
      @topentries_photo_0_to_2 = Contest.find(:all,:select => "a.*, b.*, a.id as contest_id, a.first_name as contest_fname" ,:joins=>"as a, user_contents as b",
        :conditions => "a.moment_id = b.id and a.moment_type = 'image' and a.contest_phase='phase#{@contest_phase}' and a.status='active' ", :order => "a.votes desc", :limit =>"10")
    ...
    @topentries_video_0_to_2 = Contest.find(:all,:select => "a.*, b.*, a.id as contest_id, a.first_name as contest_fname" ,:joins=>"as a, user_contents as b",
      :conditions => "a.moment_id = b.id and a.moment_type = 'video' and a.contest_phase='phase#{@contest_phase}' and a.status='active'", :order => "a.votes desc", :limit =>"#{limit}")
  ...
  def terms_conditions
    ...
    @contest_phase=Base64.decode64(params[:p])
    @contest=ContestDetail.find(@contest_phase)
    @current_page = 'contest'
  ...
  def manage_terms_conditions
    ...
    session[:contest_phase] = params[:p]
  end
  ...
  def contest_login
    ...
    if session[:hmm_user]
      ...
      redirect_to "/familywebsite_contest/contest_chapter/#{@hmm_user.family_name}?p=#{params[:p]}"
    end
    #render :layout => true
  end
  ...
  def authenticate_contestlogin
    ...
    if is_userlogged_in?
      if logged_in_hmm_user.e_user_status == 'unblocked'
        ...
        session[:contest_phase] = params[:contest_id]
            ...
            if(logged_in_hmm_user.v_e_mail=="Please update email")
              ...
              if(flag==1 && request.request_uri != "/customers/upgrade/platinum_account?msg=payment_req")
                ...
                if(logged_in_hmm_user.family_name=='' || logged_in_hmm_user.family_name==nil)
                  ...
                  hmmuser_family.save
                  redirect_to "/familywebsite_contest/contest_chapter/#{logged_in_hmm_user.family_name}?p=#{session[:contest_phase]}"
                else
  ...
  def create_vote
    ...
    if (session[:hmm_user].nil?)
      ...
      if @contest_vote <= 0
        ...
        if @vote.save
          ...
          if(moment_type=="video")
            redirect_to :controller => "familywebsite_contest", :action => 'videomoment_vote',:id => params[:id],:p=>params[:p]
          else
            redirect_to :controller => "familywebsite_contest", :action => 'moments_vote',:id => params[:id],:p=>params[:p]
          end
        else
          $notice_vote
      ...
      redirect_to :action => 'vote_cal', :id => params[:id], :contest_id => params[:contest_id], :p => params[:p]
    end
  end
  ...
  def vote_cal
    ...
    if (@contest_vote1 <= 0 and @contest_vote2 <= 0)
      ...
      @vote['vote_date']= Time.now
      @vote['email_id'] = logged_in_hmm_user.v_e_mail
      @vote['conformed'] = 'yes'
      if @vote.save
        ...
        if contest_typ == 'image'
          redirect_to  :action => 'moments_vote', :id => params[:id],:p=>params[:p]
        else if contest_typ == 'video'
            redirect_to  :action => 'videomoment_vote',:id => params[:id],:p=>params[:p]
          end
        end
      else
        $notice_vote
        flash[:notice_vote] = 'Your was not submitted due to technical issue... Please try again!!'
        if contest_typ == 'image'
          redirect_to  "/familywebsite_contest/moments_vote/#{params[:id]}?p=#{params[:p]}"
        else if contest_typ == 'video'
            redirect_to  "/familywebsite_contest/videomoment_vote/#{params[:id]}?p=#{params[:p]}"
          end
        end
      end
  ...
  def manage_create_vote
    ...
    if (session[:hmm_user].nil?)
      ...
      if @contest_vote <= 0
        ...
        unid=contestuuid[0].uid+""+"#{contestvote_next_id}"
        #inserting new values into contest vote table
        contest = ContestDetail.find(Base64.decode64(params[:p]))
        contest_name = contest.name
        contest_phase = contest.id
        @vote = ContestVotes.new()
        @vote['uid']=@uid
        ...
        if @vote.save
          ...
          Postoffice.deliver_voteconf(params[:email], contestid, unid, moment_filename, contestfname, user_fname, user_lname, moment_type, paths.proxyname, momentid, moment_url, contest_phase, contest_name)
          if(moment_type=="video")
            redirect_to "/familywebsite_contest/manage_videomoment_vote/#{params[:id]}?p=#{params[:p]}"
          else
            redirect_to "/familywebsite_contest/manage_moments_vote/#{params[:id]}?p=#{params[:p]}"
          end
        else
          $notice_vote
          flash[:notice_vote] = 'Your was not submitted due to technical issue... Please try again!!'
      ...
      redirect_to "/familywebsite_contest/manage_vote_cal/#{params[:id]}?contest_id=#{params[:contest_id]}&&p=#{params[:p]}"
    end
  end
  ...
  def manage_momentdetails
    ...
    @contest_phase=Base64.decode64(params[:p])
    @contest_details=ContestDetail.find(@contest_phase)
    @moment_details = UserContent.find(params[:contest_id])
    if(params[:page]==nil || params[:page]=='' || params[:hmm]==nil)
      @counts =  Contest.count(:all, :conditions => " moment_type='image' and status='active' and contest_phase='phase#{@contest_phase}'",:order =>'first_name  asc')
      @allrecords =  Contest.find(:all, :conditions => " moment_type='image' and status='active'and contest_phase='phase#{@contest_phase}'" ,:order =>'first_name  asc')
      page=0

      @user_content =Contest.paginate  :per_page => 1, :page => params[:page], :conditions => "moment_type='image' and status='active' and contest_phase='phase#{@contest_phase}'",:order =>'first_name  asc'
  ...
  def contest_chapter
    ...
    @tags_pages  = Tag.paginate :per_page => items_per_page,:page => params[:page], :order => sort,
      :select => "a.id, a.tag_type, a.img_url, a.v_chapimage, a.v_tagname, a.d_updateddate", :joins => "as a, user_contents as b",
      :conditions => "a.id = b.tagid and b.e_filetype = '#{session[:contests_type]}' and a.uid='#{uid}' and a.status='active' and (a.tag_type ='#{@album_list}' or a.tag_type = 'chapter' or a.tag_type = 'mobile_uploads') ",
      :group => "a.id"
  ...
  def contest_sub_chapter
    ...
    if (params[:tagtype]=='chapter' || params[:tagtype]=='mobile_uploads')

      @subchaps = SubChapter.paginate :per_page => items_per_page,:page => params[:page], :order => sort, :select => "a.id, a.v_image, a.img_url, a.sub_chapname, a.d_updated_on", :joins => "as a, user_contents as b", :conditions => "a.id = b.sub_chapid and b.e_filetype = '#{session[:contests_type]}' and a.tagid=#{params[:tagid]} and a.status='active' ", :group => "a.id" # ,:order => " #{@sort.to_s} #{@order.to_s} "
      ...
      @subchapter = SubChapter.find(:all, :select => "a.id, a.v_image, a.img_url, a.sub_chapname, a.d_updated_on, b.id as user_content_id", :joins => "as a, user_contents as b", :conditions => "a.id = b.sub_chapid and b.e_filetype = '#{session[:contests_type]}' and a.tagid=#{params[:tagid]} and a.status='active' ", :group => "a.id")
      @hmmuser = HmmUser.find(:all, :conditions => "id='#{uid}'")
      #      puts @subchapter[0]['id']
      if(countcheck==1)
        #        gallery = Galleries.find(:all, :select => "a.*, b.id as user_content_id", :joins => "as a, user_contents as b", :conditions=>"a.subchapter_id = '#{@subchapter[0]['id']}' and a.subchapter_id = '#{@subchapter[0]['id']}'")
        redirect_to :controller => "familywebsite_contest" , :action => 'contest_gallery', :id => @hmmuser[0]['family_name'], :subchapid => @subchapter[0]['id']
      ...
      if(@content_count >0)
        ...
        redirect_to :controller=>'familywebsite_contest' , :action=>'contest_moments' ,:id =>params[:id],:moment_id => @gallery_id
      else
        flash[:notice] = 'You have no moments'
        @tag_id = params[:tagid]
        redirect_to :controller=>'familywebsite_contest' , :action=>'no_moments' ,:id =>params[:id],:moment_id => 0
      end
    end
  ...
  def contest_gallery
    ...
    @sub_chapters_gallerie = Galleries.paginate :per_page => items_per_page,:page => params[:page], :order => sort,
      :select => "a.id, a.v_gallery_image, a.img_url, a.v_gallery_name, a.d_gallery_date", :joins => "as a, user_contents as b",
      :conditions => "a.id = b.gallery_id and b.e_filetype = '#{session[:contests_type]}' and a.subchapter_id=#{params[:subchapid]} and a.status='active' and a.e_gallery_type='#{session[:contests_type]}'",
      :group => "a.id"
    ...
    puts countcheck
    if(countcheck==1)
      redirect_to :controller => "familywebsite_contest" , :action => 'contest_moments', :id => @hmmuser[0]['family_name'], :moment_id => @sub_chapters_gallerie[0]['id']
    else
      render :layout => true
    end
  ...
  def contest_moments
    ...
    @current_page = 'contest'
    @hmm_user=HmmUser.find(session[:hmm_user])
    @contest_phase=Base64.decode64(session[:contest_phase])
    #    @contest_phase = session[:contest_phase]
    @contest_details=ContestDetail.find(@contest_phase)
  ...
  def submit_entry
    ...
    @user_data = UserContent.find(:first,:conditions=>"id=#{params[:moment_id]} && uid=#{logged_in_hmm_user.id}")
    @journal  = Blog.first(:conditions => "blog_type_id = #{params[:moment_id]}")
    cur_date=Date.today
    @contest_details = ContestDetail.find(:all,:conditions=>"start_date<'#{cur_date}' and end_date>'#{cur_date}'")
  end
