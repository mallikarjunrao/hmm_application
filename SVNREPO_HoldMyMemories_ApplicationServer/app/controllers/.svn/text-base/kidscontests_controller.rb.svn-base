class KidscontestsController < ApplicationController
  layout "application"
  include KidscontestsHelper
  include SortHelper
  helper :sort
  include SortHelper
  ...
  def login_check #user login check for manage family website
    unless session[:hmm_user]
      ``
      flash[:notice] = 'Please Login to Access your Account'
      redirect_to '/kidscontests/hmm_contest'
      return false
    else
      redirect_to :controller =>'familywebsite_kidscontests', :action => 'intro', :id => logged_in_hmm_user.family_name
    end
  end
  ...
  def hmm_contest
    ...
    @topentries_photo_0_to_2 = Contest.find(:all,:select => "a.*, b.*, a.id as contest_id, a.first_name as contest_fname" ,:joins=>"as a, user_contents as b",
      :conditions => "a.moment_id = b.id and a.moment_type = 'image' and a.contest_phase='#{contest_phase}' and a.status='active' and age_groups='0-2'", :order => "a.votes desc", :limit =>"2")

    logger.info "=======11111111111============"
    logger.info @topentries_photo_0_to_2.inspect
    logger.info @topentries_photo_0_to_2.size
    logger.info "=======11111111111============"
    ...
    logger.info "=======123============"
    logger.info @topentries_photo1.inspect
    logger.info contest_phase.inspect
    logger.info @topentries_photo1.size
    logger.info "=======123============"
    ...
    @topentries_photo_3_to_5 = Contest.find(:all,:select => "a.*, b.*, a.id as contest_id, a.first_name as contest_fname" ,:joins=>"as a, user_contents as b",
      :conditions => "a.moment_id = b.id and a.moment_type = 'image' and a.contest_phase='#{contest_phase}'and a.age_groups='3-5' and a.status='active'", :order => "a.votes desc", :limit =>"2")
    ...
    logger.info "==================="
    logger.info @topentries_photo2.inspect
    logger.info @topentries_photo2.size
    ...
    @topentries_photo_6_to_9 = Contest.find(:all,:select => "a.*, b.*, a.id as contest_id, a.first_name as contest_fname" ,:joins=>"as a, user_contents as b",
      :conditions => "a.moment_id = b.id and a.moment_type = 'image' and a.contest_phase='#{contest_phase}'and a.age_groups='6-9' and a.status='active'", :order => "a.votes desc", :limit =>"2")
    ...
    logger.info "==================="
    logger.info @topentries_photo3.inspect
    logger.info @topentries_photo3.size
    ...
    @topentries_photo_10_to_17 = Contest.find(:all,:select => "a.*, b.*, a.id as contest_id, a.first_name as contest_fname" ,:joins=>"as a, user_contents as b",
      :conditions => "a.moment_id = b.id and a.moment_type = 'image' and a.contest_phase='#{contest_phase}'and a.age_groups='10-17' and a.status='active'", :order => "a.votes desc", :limit =>"2")
    ...
    @topentries_video_0_to_2 = Contest.find(:all,:select => "a.*, b.*, a.id as contest_id, a.first_name as contest_fname" ,:joins=>"as a, user_contents as b",
      :conditions => "a.moment_id = b.id and a.moment_type = 'video' and a.contest_phase='#{contest_phase}'and a.age_groups='0-2' and a.status='active'", :order => "a.votes desc", :limit =>"2")

    @topentries_video1 = Array.new
    @topentries_video_0_to_2.each do |video|
      @topentries_video1 << video
    end
    logger.info "==================="
    logger.info @topentries_video1.inspect
    @topentries_video_3_to_5 = Contest.find(:all,:select => "a.*, b.*, a.id as contest_id, a.first_name as contest_fname" ,:joins=>"as a, user_contents as b",
      :conditions => "a.moment_id = b.id and a.moment_type = 'video' and a.contest_phase='#{contest_phase}'and a.age_groups='3-5' and a.status='active'", :order => "a.votes desc", :limit =>"2")

    @topentries_video2 = Array.new
    @topentries_video_3_to_5.each do |video|
      @topentries_video2 << video
    end
    logger.info "==================="
    logger.info @topentries_video2.inspect

    @topentries_video_6_to_9 = Contest.find(:all,:select => "a.*, b.*, a.id as contest_id, a.first_name as contest_fname" ,:joins=>"as a, user_contents as b",
      :conditions => "a.moment_id = b.id and a.moment_type = 'video' and a.contest_phase='#{contest_phase}'and a.age_groups='6-9' and a.status='active'", :order => "a.votes desc", :limit =>"2")

    @topentries_video3 = Array.new
    @topentries_video_6_to_9.each do |video|
      @topentries_video3<< video
    end

    logger.info "==================="
    logger.info @topentries_photo3.inspect

    @topentries_video_10_to_17 = Contest.find(:all,:select => "a.*, b.*, a.id as contest_id, a.first_name as contest_fname" ,:joins=>"as a, user_contents as b",
      :conditions => "a.moment_id = b.id and a.moment_type = 'video' and a.contest_phase='#{contest_phase}'and a.age_groups='10-17' and a.status='active'", :order => "a.votes desc", :limit =>"2")

    @topentries_video4 = Array.new
    @topentries_video_10_to_17.each do |video|
      @topentries_video4 << video
    end
    logger.info "==================="
    logger.info @topentries_photo4.inspect
  ...
  def authenticate
    ...
    if is_userlogged_in?
      if logged_in_hmm_user.e_user_status == 'unblocked'
        ...
        redirect_to :controller => 'familywebsite_kidscontests', :action =>'contest_chapter', :id => logged_in_hmm_user.family_name
      else
        reset_session
  ...
  def contest_sub_chapter
    ...
    if(countcheck==1)
      ...
      redirect_to :controller => "kidscontests" , :action => 'contest_gallery', :id => @sub_chapters[0]['id']
  ...
  def contest_gallery
    ...
    if(countcheck==1)
      redirect_to :controller => "kidscontests" , :action => 'contest_moments', :id =>actualid
    else
      render :layout => true
    end
    ...
    if @authenticate_unid == 1
      ...
      if is_userlogged_in?
        if logged_in_hmm_user.e_user_status == 'unblocked'
          ...
          redirect_to :controller => 'kidscontests', :action =>'contest_upload'
        end
        #redirect_to :action => authenticate_pass, :username => username, :password =>password
      end
    else
      redirect_to :controller => 'kidscontests', :action => 'contest_login'
    end
  end
  ...
  def create_contest
    ...
    @contest['contest_title']="Cute Kids Contest"
    ...
    if @journal.save
      ...
      redirect_to :controller => :kidscontests, :action => :contestmoment_show
    else
      redirect_to :controller => :kidscontests, :action => :contest_chapter
    end
  ...
  def momentdetails
    ...
    @contest_description =  "Hello Friends!\n\nI have entered in HoldMyMemories.com Cute Kids Contest Photo and Video Contest. Please help #{entrants_name} to win by clicking on the above link and casting your vote!"
    @contest_title="Cute Kids Contest Photo and Video Contest.Brought to you by HoldMyMemories.com."
  ...
  def videomoment_details
    ...
    entrants_name=@contests_fname[0].first_name
    @contest_description =  "Hello Friends!\n\nI have entered in HoldMyMemories.com Cute Kids Contest 2010 Video Contest. Please help #{entrants_name} to win by clicking on the above link and casting your vote!"
    @contest_title="Cute Kids Contest Video Contest. Brought to you by HoldMyMemories.com."
  ...
  def vote
    ...
    @contest_image="#{@moment_details[0].img_url}/user_content/photos/journal_thumb/#{@moment_details[0].v_filename}"
    entrants_name=@contests_fname[0].first_name
    @contest_description =  "Hello Friends!\n\nI have entered in HoldMyMemories.com Cute Kids Contest 2010 Photo and Video Contest. Please help #{entrants_name} to win by clicking on the above link and casting your vote!"
    @contest_title="Cute Kids Contest Photo and Video Contest.Brought to you by HoldMyMemories.com."
    render :layout => false
  end
  ...
  def create_vote
    ...
    @contest_vote = ContestVotes.count(:conditions => "contest_id = '#{contetsid}' and email_id = '#{params[:email]}' and vote_date ='#{my_date_time}'")
    if @contest_vote <= 0
    ...
    if(@contest_email_chk>0)
      ...
      if @authenticate_vote_count == 1
        ...
        if(contest_votes_check > 0)
          $notice_voteconform
          res= Base64.encode64("Our records indicate you have already confirmed your vote for this entry with email address #{contest_votes[0].email_id}.  Voters may only vote once per entry.  Thank-you for your vote!")
          flash[:notice_voteconform]=res
          redirect_to(:action => "vote_exist", :xc454345556 => res)
        else
          ...
          flash[:notice_voteconform] = res
          redirect_to :action => 'vote_thanks'
        end
      else
        $notice_voteconform
        res = 'Our records indicate you have already voted for this entry.  Voters may only vote once per entry.  Thank-you for your vote! '
  ...
  def share_facebook
    ...
    @contest_description =  "Hello Friends!\n\nI have entered in HoldMyMemories.comCute Kids Contest Photo and Video Contest Please help #{entrants_name} to win by clicking on the above link and casting your vote!"
    @contest_title="Cute Kids Contest Photo and Video Contest.Brought to you by HoldMyMemories.com."
  end
  ...
  def excel_report
    ...
    #CSV_FILE_PATH = File.join("#{RAILS_ROOT}/app", "output.xls")


    #    book = Spreadsheet.open '/home/shrikanth.hathwar/Downloads/Declaration Form 2011-12.xls'
    #    sheet1 =book.worksheet('HT0000')
    # row = sheet1.each do |row|
    #
    #    logger.info(row)
    #
    #
    #  end
    oo = Excel.new("/home/shrikanth.hathwar/Documents/1.xls")
    oo.default_sheet = oo.sheets.first
    2.upto(200) do |line|
      date       = oo.cell(line,'a')
      #      date       = oo.cell(line,'a')
      #      date       = oo.cell(line,'a')
      dates=date.split("HMM")
      val=Subscription.new
      val.subscriptions=dates[2]
      #      val.fname=fname
      #      val.lname=lname
      val.save
      logger.info(date)
...
#    for sub in subs
#      hmmuser=HmmUser.find(:first,:conditions=>"id=#{sub.subscriptions}")
#      logger.info("user_id=>#{hmmuser.id}")
#      hmmuser.account_expdate="2011-07-18"
...
#        studio_commission.payment_recieved_on="2011-06-18"
...
#      payment_count.recieved_on  ="2011-06-18"
