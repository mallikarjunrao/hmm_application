class FamilywebsiteKidscontestsController < ApplicationController
   include FamilywebsiteKidscontestsHelper
  ...
  def contest_login
    ...
    if session[:hmm_user]
      ...
      redirect_to "/familywebsite_kidscontests/contest_chapter/#{@hmm_user.family_name}"
    end
    #render :layout => true
  end
            ...
            if(logged_in_hmm_user.v_e_mail=="Please update email")
              ...
              if(flag==1 && request.request_uri != "/customers/upgrade/platinum_account?msg=payment_req")
                ...
                if(logged_in_hmm_user.family_name=='' || logged_in_hmm_user.family_name==nil)
                  ...
                  hmmuser_family.save
                  redirect_to :controller=>'familywebsite_kidscontests',:action=>'contest_chapter',:id=>logged_in_hmm_user.family_name, :contest => params[:contest]
                else
...
paths=ContentPath.find(:first,:conditions=>"status='active'")
      ...
      @contest_vote = ContestVotes.count(:all, :conditions => "contest_id = '#{contetsid}' and email_id = '#{params[:email]}' and vote_date ='#{my_date_time}'")
      if @contest_vote <= 0
        ...
        if @vote.save
          ...
          if(moment_type=="video")
            redirect_to :controller => "familywebsite_kidscontests", :action => 'videomoment_vote',:id => params[:id]
          else
            redirect_to :controller => "familywebsite_kidscontests", :action => 'moments_vote',:id => params[:id]
          end
        else
          $notice_vote
    ...
    if (session[:hmm_user].nil?)
      ...
      if @contest_vote <= 0
        ...
        if @vote.save
          $notice_vote1
          flash[:notice_vote] = 'Thank-you for casting your vote.  You will shortly receive an email verification that you must accept for your vote to be counted.  This helps us make sure that all votes are valid votes.  Your email address will not be shared with any other company.'
          Postoffice.deliver_kidsvoteconform(params[:email],contetsid, unid,moment_filename,contetsfname,user_fname,user_lname,moment_type,paths.proxyname,momentid,contest_phase)
          if(moment_type=="video")
            redirect_to :controller => "familywebsite_kidscontests", :action => 'manage_videomoment_vote',:id => params[:id]
          else
            redirect_to :controller => "familywebsite_kidscontests", :action => 'manage_moments_vote',:id => params[:id]
          end
        else
          $notice_vote
          flash[:notice_vote] = 'Your was not submitted due to technical issue... Please try again!!'
  ...
  def manage_vote_cal
    ...
    contest_typ = @contest.moment_type
    my_date_time = Date.today
    @contest_vote1 = ContestVotes.count(:all, :conditions => "contest_id = '#{contetsid}' and hmm_voter_id = '#{logged_in_hmm_user.id}' and vote_date = '#{my_date_time}'")
    @contest_vote2 = ContestVotes.count(:all, :conditions => "contest_id = '#{contetsid}' and email_id = '#{logged_in_hmm_user.v_e_mail}' and vote_date = '#{my_date_time}'")
    puts @contest_vote2
    if (@contest_vote1 <= 0 and @contest_vote2 <= 0)
      #calculating next id for the contest vote table
      @contestvote_max =  ContestVotes.find_by_sql("select max(id) as n from contest_votes")
  ...
  def contest_sub_chapter
    ...
    if (params[:tagtype]=='chapter')
      ...
      if(countcheck==1)
        ...
        redirect_to :controller => "familywebsite_kidscontests" , :action => 'contest_gallery', :id => @hmmuser[0]['family_name'], :subchapid => @subchapter[0]['id']
      ...
      if(@content_count >0)
        ...
        redirect_to :controller=>'familywebsite_kidscontests' , :action=>'contest_moments' ,:id =>params[:id],:moment_id => @gallery_id
      else
        flash[:notice] = 'You have no moments'
        @tag_id = params[:tagid]
        redirect_to :controller=>'familywebsite_kidscontests' , :action=>'no_moments' ,:id =>params[:id],:moment_id => 0
      end
    end
  ...
  def contest_gallery
    ...
    puts countcheck
    if(countcheck==1)
      redirect_to :controller => "familywebsite_kidscontests" , :action => 'contest_moments', :id => @hmmuser[0]['family_name'], :moment_id => @sub_chapters_gallerie[0]['id']
    else
      render :layout => true
    end
  ...
  def select_contestmoment
    issubmitted = Contest.find_by_sql("select *  from contests where contests.moment_id='#{params[:momentid]}' and contests.uid=#{session[:hmm_user]} and contest_phase = '#{contest_phase}'")
    ...
    if(issubmitted.length == 0)
      @contest_namecheck = Contest.find_by_sql("select *  from contests where contests.first_name='#{params[:fname]}' and contests.uid='#{session[:hmm_user]}' and moment_type = '#{params[:type]}' and contest_phase = '#{contest_phase}' and status != 'inactive'")
      ...
      if (@contest_namecheck.length == 0)
        ...
        @contest['contest_title']="Home For The Holidays2010 Contest"
