class AmericanFamilyController < ApplicationController

  layout "application"
  include AmericanFamilyHelper
  include SortHelper
  helper :sort
  include SortHelper
  ...
  ssl_required :contest_login,:authenticate
  ssl_allowed  :initialize,:list,:show,:new,:create,:edit,:update,:destroy,:contest_terms_conditions,:hmm_contest,:phase2_contest,:evaluate_terms,:contest_chapter,:contest_sub_chapter,:contest_moments,:contest_gallery,:validate_familyname,:conform_email,:validate,:contest_upload,:create_contest,:get_memories_list,:contestmoment_show,:select_contestmoment,:moments_vote,:videomoment_vote,:momentdetails,:momentdetails1,:momentdetails_old_contest,:videomoment_details,:videomoment_old_contest,:vote,:create_vote,:forgot_password,:password_status,:validate_email,:mycontest_moments,:mymemorable_moments,:conform_vote,:authenticate_vote,:contests_select,:cutekid_conteste,:previous_contest,:logged_moments_vote,:vote_cal,:logged_videomoment_vote,:logged_terms_conditions,:facebook_logout,:myspace_invite,:import,:import_form,:revision,:vote_request,:validate_babyname,:moments_entrants,:videomoment_entrants,:hmm_contest_cutekid2010,:facebook_share,:hmm_contest_new,:enter_contest,:share_facebook,:share_via_email,:share_via_email_sub,:thank_you_sharing,:vote_facebook,:create_sharecount,:fblike,:like,:s121_facebook_like,:s121_facebook_fblike,:rules,:ruleslist

  before_filter :change_ssl, :only => [:initialize,:list,:show,:new,:create,:edit,:update,:destroy,:contest_terms_conditions,:hmm_contest,:phase2_contest,:evaluate_terms,:contest_chapter,:contest_sub_chapter,:contest_moments,:contest_gallery,:validate_familyname,:conform_email,:validate,:contest_upload,:create_contest,:get_memories_list,:contestmoment_show,:select_contestmoment,:moments_vote,:videomoment_vote,:momentdetails,:momentdetails1,:momentdetails_old_contest,:videomoment_details,:videomoment_old_contest,:vote,:create_vote,:forgot_password,:password_status,:validate_email,:mycontest_moments,:mymemorable_moments,:conform_vote,:authenticate_vote,:contests_select,:cutekid_conteste,:previous_contest,:logged_moments_vote,:vote_cal,:logged_videomoment_vote,:logged_terms_conditions,:facebook_logout,:myspace_invite,:import,:import_form,:revision,:vote_request,:validate_babyname,:moments_entrants,:videomoment_entrants,:hmm_contest_cutekid2010,:facebook_share,:hmm_contest_new,:enter_contest,:share_facebook,:share_via_email,:share_via_email_sub,:thank_you_sharing,:vote_facebook,:create_sharecount,:fblike,:like,:s121_facebook_like,:s121_facebook_fblike,:rules,:ruleslist]
  ...
  def login_check #user login check for manage family website
    unless session[:hmm_user]
      ``
      flash[:notice] = 'Please Login to Access your Account'
      redirect_to '/american_family/hmm_contest'
      return false
    else
      redirect_to :controller =>'familywebsite_american_family', :action => 'intro', :id => logged_in_hmm_user.family_name
    end
  end
  ...
  def authenticate
    ...
    if is_userlogged_in?
      if logged_in_hmm_user.e_user_status == 'unblocked'
        ...
        redirect_to :controller => 'familywebsite_american_family', :action =>'contest_chapter', :id => logged_in_hmm_user.family_name
      else
        reset_session
  ...
  def contest_sub_chapter
    ...
    if(countcheck==1)
      ...
      redirect_to :controller => "american_family" , :action => 'contest_gallery', :id => @sub_chapters[0]['id']
  ...
  def contest_gallery
    ...
    if(countcheck==1)
      redirect_to :controller => "american_family" , :action => 'contest_moments', :id =>actualid
    else
      render :layout => true
    end
    ...
    if @authenticate_unid == 1
      ...
      if is_userlogged_in?
        if logged_in_hmm_user.e_user_status == 'unblocked'
          ...
          redirect_to :controller => 'american_family', :action =>'contest_upload'
        end
        #redirect_to :action => authenticate_pass, :username => username, :password =>password
      end
    else
      redirect_to :controller => 'american_family', :action => 'contest_login'
    end
  end
  ...
  def create_contest
    ...
    @contest['contest_title']="All American FAmily"
    ...
    if @journal.save
      ...
      redirect_to :controller => :american_family, :action => :contestmoment_show
    else
      redirect_to :controller => :american_family, :action => :contest_chapter
    end
  ...
  def momentdetails
    ...
    @contest_description =  "Hello Friends!\n\nI have entered in HoldMyMemories.com All American Family Photo and Video Contest. Please help #{entrants_name} to win by clicking on the above link and casting your vote!"
    @contest_title="All American Family Photo and Video Contest.Brought to you by HoldMyMemories.com."
  ...
  def videomoment_details
    ...
    entrants_name=@contests_fname[0].first_name
    @contest_description =  "Hello Friends!\n\nI have entered in HoldMyMemories.com All American Family Video Contest. Please help #{entrants_name} to win by clicking on the above link and casting your vote!"
    @contest_title="All Amrican Family Video Contest. Brought to you by HoldMyMemories.com."
    ...
    if @contest_vote <= 0
      ...
      if @vote.save
        ...
        if(moment_type=="video")
          redirect_to :controller => "american_family", :action => 'videomoment_vote'
        else
          redirect_to :controller => "american_family", :action => 'moments_vote'
        end
      else
        $notice_vote
