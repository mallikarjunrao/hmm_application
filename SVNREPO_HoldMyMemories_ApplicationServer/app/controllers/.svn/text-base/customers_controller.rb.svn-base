...
#  ssl_allowed  :newcupon_account
  ...
  def new
    if(session[:hmm_user].nil?)
      ...
      redirect_to "/user_account/login/?msg=1"
  ...
  def new1
    session[:v_fname]=params[:v_fname]
    session[:v_lname]=params[:v_lname]
    session[:marital_status]=params[:marital_status]
    session[:e_sex]=params[:e_sex]
    session[:v_zip]=params[:v_zip]
    session[:v_security_q]=params[:v_security_q]
    session[:v_security_a]=params[:v_security_a]
    session[:v_abt_me]=params[:v_abt_me]
    session[:v_link1]=params[:v_link1]
    session[:v_link2]=params[:v_link2]
    session[:v_link3]=params[:v_link3]
    session[:errorusername]=params[:errorusername]
    session[:v_city]=params[:v_city]
    session[:v_state]=params[:v_state]
    session[:v_e_mail]=params[:v_e_mail]
    session[:family_name]=params[:family_name]
    session[:v_user_name]=params[:v_user_name]

    if(params[:payment_type]=="no")
      redirect_to "#{request_server_url}/customers/new/"
    else
      redirect_to "#{request_server_url}/customers/new/#{params[:payment_type]}"
    end
  end
  ...
  def other_details
    ...
    session[:v_fname]=""
    session[:v_lname]=""
    session[:marital_status]=""
    session[:e_sex]= ""
    session[:v_zip]=""
    session[:v_security_q]=""
    session[:v_security_a]=""
    session[:v_abt_me]=""
    session[:v_link1]=""
    session[:v_link2]=""
    session[:v_link3]=""
  ...
  def create
    ...
    if @hmm_user.save
      ...
      link= "#{request_server_url}/"+params[:hmm_user][:family_name]
  ...
  def validate
    ...
    user = HmmUser.count(:all, :conditions => "v_user_name='#{username}'")
    if user > 0
  ...
  def validate_familyname
    ...
    if(params[:familyname]=='' || params[:familyname]==nil)
      family = 0
    else
      fname=params[:familyname]
      fname.gsub("'","")
      family = HmmUser.count(:all, :conditions => "family_name='#{fname}' || alt_family_name='#{fname}'")
    end
    if family > 0
    ...
    render :partial=>'message_family' , :layout => false
  end

  def validate_email
    color = 'red'
    email = params[:email]
    mail = HmmUser.count(:all, :conditions => "v_e_mail='#{email}'")
    if mail > 0
    ...
    render :partial=>'message_email' , :layout => false
  end
  ...
  def chapterindex
    ...
    if(session[:friend]!='')
      ...
      @friend=HmmUser.find(session[:friend])
    ...
    @tags = Tag.paginate  :per_page => items_per_page, :page => params[:page], :order => sort, :conditions => "uid='#{uid}' and status='active' #{e_access}" # ,:order => " #{@sort.to_s} #{@order.to_s} "
  ...
  def chapters_display
    ...
    @user_rec = HmmUser.find(:all, :conditions => "id='#{logged_in_hmm_user.id}'")
    @userid = logged_in_hmm_user
    @flag = 0
  ...
  def chapter_gallery
    ...
    @user_content = UserContent.paginate :per_page => items_per_page, :page => params[:page], :conditions=>"gallery_id=#{params[:id]}  and status='active'", :order => sort
  ...
  def fnf_index
    ...
    if( params[:query]=="" && params[:query1]=="" && params[:query_hmm_user] =="" )
      ...
      @hmm_users = HmmUser.paginate :per_page => items_per_page, :page => params[:page],:joins=>"as b , family_friends  as a ", :select => "*", :order => sort,
 ...
 select
  ...
  def album_moments
    @userid = params[:id]
    @results =  Tag.find(:all, :conditions=> "id=#{params[:tagid]} and uid=#{@userid}")
    render :layout => false
  end
  ...
  def fnf_show
    ...
    @fnf_reqa = HmmUser.paginate :per_page => items_per_page, :page => params[:page] ,:joins=>"as b , family_friends  as a ", :select => "*", :conditions => "a.fnf_category =#{params[:id]} and a.uid=#{logged_in_hmm_user.id} && b.id=a.fid && a.status='accepted' and a.block_status='unblock' #{fnf_search}"
    @hmm_user=HmmUser.find(session[:hmm_user])
    @fnf_req = HmmUser.find_by_sql("SELECT b.*, a.id as frnid, a.* FROM family_friends  as a ,hmm_users as b  WHERE (a.fid=#{logged_in_hmm_user.id} and a.uid=b.id and a.status='pending')")

    @fnf_groups = FnfGroups.paginate :per_page => 10, :page => params[:page] ,:conditions => " uid=#{logged_in_hmm_user.id} "
  ...
  def change_group_value
    ...
    flash[:notice_accept] = 'The group changed successfully !!!'
  ...
  def shareJournal_list
    ...
    @share_list = ShareJournal.find(:all, :joins => "as a, hmm_users as b", :select => "*", :conditions => "b.id=#{logged_in_hmm_user.id} and b.v_e_mail=a.e_mail and  DATE_FORMAT(a.expiry_date,'%Y-%m-%d') >= curdate()")
  ...
  def facebook_share_video1
    @usercontent = UserContent.find(params[:id])
    render :layout => false
  end
  def facebook_share_video3
    @usercontent = UserContent.find(params[:id])
    render :layout => false
  end

  def validate_wpf_username
    if params[:username]
      username = params[:username]
      user = HmmUser.find_all_by_v_user_name(username)
      if user.size > 0
        @valid_username = false
        redirect_to :action => 'new', :id => 'reg_un'
      else
        @valid_username=true
        redirect_to :action =>'new', :id => 'notreg_un'
      end
    else if params[:email]
        email = params[:email]
        mail = HmmUser.find_all_by_v_e_mail(email)
        if mail.size > 0
          redirect_to :action => 'new', :id => 'reg_em'
        else
          redirect_to :action =>'new', :id => 'notreg_em'
        end
      else if params[:familyname]
          familyname = params[:familyname]
          family = HmmUser.find_all_by_family_name(familyname)
          if family.size > 0
            redirect_to :action => 'new', :id => 'reg_fn'
          else
            redirect_to :action =>'new', :id => 'notreg_fn'
          end
        end
      end
    end
  ...
  def validate_wpf_uniques(username)
  ...
  def fileext
    @res= "23423423fewfewfew".to_i
    #@res=Integer(rotate_degree)
    render :layout => false
  end
  ...
  def verify_gift_card
    ...
    unless params[:coupon_id].blank?
      @coupon_count = Cupon.count(:all, :conditions => "unid='#{params[:coupon_id]}'")
      unless @coupon_count==0
        @hmmuser_count = HmmUser.count(:all, :conditions => "cupon_no='#{params[:coupon_id]}'")
        if @hmmuser_count ==0
          @cp_cnt = Cupon.count(:all,:conditions => "unid='#{params[:coupon_id]}'")
          @cupon = Cupon.find(:first,:conditions => "unid='#{params[:coupon_id]}'")
          @cupon_res = Cupon.count(:all, :conditions => "id='#{@cupon.id}' and DATE_FORMAT(expire_date,'%Y-%m-%d') >= curdate() ")
          @cupon_res_start = Cupon.count(:all, :conditions => "id='#{@cupon.id}' and DATE_FORMAT(start_date,'%Y-%m-%d') <= curdate() ")
        else
          @coupon_result = 'This coupon is already been used'
        end
        #redirect_to :action => 'gift_card_result',:coupon_id => params[:coupon_id]
      else
        @coupon_result = 'Invalid Coupon'
      end
    end
