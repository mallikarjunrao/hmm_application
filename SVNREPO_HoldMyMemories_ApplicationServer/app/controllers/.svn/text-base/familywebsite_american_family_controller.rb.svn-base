class FamilywebsiteAmericanFamilyController < ApplicationController

  include FamilywebsiteAmericanFamilyHelper
  ...
  #  ssl_required :contest_login,:login,:authenticate_contestlogin
  #  ssl_allowed  :check_account,:intro,:manage_intro,:terms_conditions,:manage_terms_conditions,:moments_vote,:manage_moments_vote,:vote,:create_vote,:vote_cal,:manage_vote,:manage_create_vote,:videomoment_vote,:manage_videomoment_vote,:momentdetails,:momentdetails_old_contest,:manage_momentdetails,:videomoment_details,:videodetails_old_contest,:manage_videomoment_details,:contest_terms_conditions,:contest_chapter,:contest_sub_chapter,:no_moments,:contest_gallery,:contest_moments,:previous_contest,:select_contestmoment,:alert_messages,:get_memories_list,:send_voteconfirm,:videomoment_entrants,:moments_entrants,:specific_rules
  #  before_filter :change_ssl, :only => [:check_account,:intro,:manage_intro,:terms_conditions,:manage_terms_conditions,:moments_vote,:manage_moments_vote,:vote,:create_vote,:vote_cal,:manage_vote,:manage_create_vote,:videomoment_vote,:manage_videomoment_vote,:momentdetails,:momentdetails_old_contest,:manage_momentdetails,:videomoment_details,:videodetails_old_contest,:manage_videomoment_details,:contest_terms_conditions,:contest_chapter,:contest_sub_chapter,:no_moments,:contest_gallery,:contest_moments,:previous_contest,:select_contestmoment,:alert_messages,:get_memories_list,:send_voteconfirm,:videomoment_entrants,:moments_entrants,:specific_rules]
  ...
  def intro
    ...
    @topentries_photo_0_to_2 = Contest.find(:all,:select => "a.*, b.*, a.id as contest_id, a.first_name as contest_fname" ,:joins=>"as a, user_contents as b",
      :conditions => "a.moment_id = b.id and a.moment_type = 'image' and a.contest_phase='#{contest_phase}' and a.status='active' ", :order => "a.votes desc", :limit =>"20")
    ...
    @topentries_video_0_to_2 = Contest.find(:all,:select => "a.*, b.*, a.id as contest_id, a.first_name as contest_fname" ,:joins=>"as a, user_contents as b",
      :conditions => "a.moment_id = b.id and a.moment_type = 'video' and a.contest_phase='#{contest_phase}' and a.status='active'", :order => "a.votes desc", :limit =>"20")
  ...
  def contest_login
    ...
    if session[:hmm_user]
      ...
      redirect_to "/familywebsite_american_family/contest_chapter/#{@hmm_user.family_name}"
    end
    #render :layout => true
  end
            ...
            if(logged_in_hmm_user.v_e_mail=="Please update email")
              ...
              if(flag==1 && request.request_uri != "/customers/upgrade/platinum_account?msg=payment_req")
                ...
                if(logged_in_hmm_user.family_name=='' || logged_in_hmm_user.family_name==nil)
                  ...
                  hmmuser_family.save
                  redirect_to :controller=>'familywebsite_american_family',:action=>'contest_chapter',:id=>logged_in_hmm_user.family_name, :contest => params[:contest]
                else
      ...
      if @contest_vote <= 0
        ...
        if @vote.save
          ...
          if(moment_type=="video")
            redirect_to :controller => "familywebsite_american_family", :action => 'videomoment_vote',:id => params[:id]
          else
            redirect_to :controller => "familywebsite_american_family", :action => 'moments_vote',:id => params[:id]
          end
        else
          $notice_vote
    ...
    if (session[:hmm_user].nil?)
      ...
      if @contest_vote <= 0
        ...
        if @vote.save
          ...
          if(moment_type=="video")
            redirect_to :controller => "familywebsite_kidscontes", :action => 'manage_videomoment_vote',:id => params[:id]
          else
            redirect_to :controller => "familywebsite_american_family", :action => 'manage_moments_vote',:id => params[:id]
          end
        else
          $notice_vote
          flash[:notice_vote] = 'Your was not submitted due to technical issue... Please try again!!'
          if(moment_type=="video")
            redirect_to :controller => "familywebsite_american_family",:action => 'manage_videomoment_vote',:id => params[:id]
          else
            redirect_to :controller => "familywebsite_american_family",:action => 'manage_moments_vote',:id => params[:id]
          end
        end
  ...
  def contest_sub_chapter
    ...
    sort_update
    params[:tagid]=CGI.escapeHTML(params[:tagid])
    params[:tagtype]=CGI.escapeHTML(params[:tagtype])
    ...
    if (params[:tagtype]=='chapter')
      ...
      if(countcheck==1)
        ...
        redirect_to :controller => "familywebsite_american_family" , :action => 'contest_gallery', :id => @hmmuser[0]['family_name'], :subchapid => @subchapter[0]['id']
      ...
      if(@content_count >0)
        ...
        redirect_to :controller=>'familywebsite_american_family' , :action=>'contest_moments' ,:id =>params[:id],:moment_id => @gallery_id
      else
        flash[:notice] = 'You have no moments'
        @tag_id = params[:tagid]
        redirect_to :controller=>'familywebsite_american_family' , :action=>'no_moments' ,:id =>params[:id],:moment_id => 0
      end
    end
  ...
  def contest_gallery
    ...
    sort_init 'd_updated_on'
    sort_update
    session[:sub_id]=CGI.escapeHTML(params[:subchapid])
    params[:sort_key]=CGI.escapeHTML(params[:sort_key])
    params[:subchapid]=CGI.escapeHTML(params[:subchapid])
    params[:sort_order]=CGI.escapeHTML(params[:sort_order])
    @hmm_user=HmmUser.find(session[:hmm_user])
    items_per_page = 9
    ...
    puts countcheck
    if(countcheck==1)
      redirect_to :controller => "familywebsite_american_family" , :action => 'contest_moments', :id => @hmmuser[0]['family_name'], :moment_id => @sub_chapters_gallerie[0]['id']
    else
      render :layout => true
    end
