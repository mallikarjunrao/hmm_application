...
#     end
    ...
    if @check_familyname1 > 0 
      ...
      if @hmm_user.update_attributes(params[:hmm_user])
       ...
       flash[:notice_step1fail] = 'Family Information was failed to update Family name already exisits..'
       redirect_to :action => 'familypage_step1'#, :id => logged_in_hmm_user.id
     end
...
#   INSERTING HMM USERS IN SHARE'S TABLE    
    if params[:user]
      ...
      for i in @hmmusers
        ...
        @share_to= i.v_e_mail
        @messageboard_link = "http://www.holdmymemories.com/my_familywebsite/messageboard/"+logged_in_hmm_user.family_name
        Postoffice.deliver_shareFamilywebsite(@share_to,@message,@share_link,logged_in_hmm_user.v_fname,logged_in_hmm_user.v_e_mail,logged_in_hmm_user.v_myimage,@password,@messageboard_link)
