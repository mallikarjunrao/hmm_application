class FamilywebsiteSeniorscontestController < ApplicationController
  ...
  def intro
    ...
    #      :conditions => "a.moment_id = b.id and a.moment_type = 'image' and a.contest_phase='#{seniorcontest_phase}' and a.status='active' and age_groups='18-25'", :order => "a.votes desc", :limit =>"2")
    ...
    #    logger.info seniorcontest_phase.inspect
  ...
  def contest_login
    ...
    if session[:hmm_user]
      ...
      redirect_to "/familywebsite_seniorscontest/contest_chapter/#{@hmm_user.family_name}"
    end
    #render :layout => true
  end
            ...
            if(logged_in_hmm_user.v_e_mail=="Please update email")
              ...
              if(flag==1 && request.request_uri != "/customers/upgrade/platinum_account?msg=payment_req")
                ...
                if(logged_in_hmm_user.family_name=='' || logged_in_hmm_user.family_name==nil)
                  ...
                  hmmuser_family.save
                  redirect_to :controller=>'familywebsite_seniorscontest',:action=>'contest_chapter',:id=>logged_in_hmm_user.family_name, :contest => params[:contest]
                else
      ...
      if @contest_vote <= 0
        ...
        if @vote.save
          ...
          if(moment_type=="video")
            redirect_to :controller => "familywebsite_seniorscontest", :action => 'videomoment_vote',:id => params[:id]
          else
            redirect_to :controller => "familywebsite_seniorscontest", :action => 'moments_vote',:id => params[:id]
          end
        else
          $notice_vote
    ...
    if (session[:hmm_user].nil?)
      ...
      if @contest_vote <= 0
        ...
        if @vote.save
          ...
          if(moment_type=="video")
            redirect_to :controller => "familywebsite_seniorscontest", :action => 'manage_videomoment_vote',:id => params[:id]
          else
            redirect_to :controller => "familywebsite_seniorscontest", :action => 'manage_moments_vote',:id => params[:id]
          end
        else
          $notice_vote
          flash[:notice_vote] = 'Your was not submitted due to technical issue... Please try again!!'
  ...
  def contest_sub_chapter
    ...
    if (params[:tagtype]=='chapter')
      ...
      if(countcheck==1)
        ...
        redirect_to :controller => "familywebsite_seniorscontest" , :action => 'contest_gallery', :id => @hmmuser[0]['family_name'], :subchapid => @subchapter[0]['id']
      ...
      if(@content_count >0)
        ...
        redirect_to :controller=>'familywebsite_seniorscontest' , :action=>'contest_moments' ,:id =>params[:id],:moment_id => @gallery_id
      else
        flash[:notice] = 'You have no moments'
        @tag_id = params[:tagid]
        redirect_to :controller=>'familywebsite_seniorscontest' , :action=>'no_moments' ,:id =>params[:id],:moment_id => 0
      end
    end
  ...
  def contest_gallery
    ...
    puts countcheck
    if(countcheck==1)
      redirect_to :controller => "familywebsite_seniorscontest" , :action => 'contest_moments', :id => @hmmuser[0]['family_name'], :moment_id => @sub_chapters_gallerie[0]['id']
    else
      render :layout => true
    end
