class FamilymemoryContestsController < ApplicationController
  layout 'family_memory'
  ...
  def contest_login
    ...
    if session[:hmm_user]
      ...
      redirect_to "/familymemory_contests/contest_chapter/#{@hmm_user.family_name}?p=#{params[:p]}"
    end
    #render :layout => true
  end
            ...
            if(logged_in_hmm_user.v_e_mail=="Please update email")
              ...
              if(flag==1 && request.request_uri != "/customers/upgrade/platinum_account?msg=payment_req")
                ...
                if(logged_in_hmm_user.family_name=='' || logged_in_hmm_user.family_name==nil)
                  ...
                  hmmuser_family.save
                  redirect_to "/familymemory_contests/contest_chapter/#{logged_in_hmm_user.family_name}?p=#{session[:contest_phase]}"
                else
      ...
      if @contest_vote <= 0
        ...
        if @vote.save
          ...
          if(moment_type=="video")
            redirect_to :controller => "familymemory_contests", :action => 'videomoment_vote',:id => params[:id],:p=>params[:p]
          else
            redirect_to :controller => "familymemory_contests", :action => 'moments_vote',:id => params[:id],:p=>params[:p]
          end
        else
          $notice_vote
    ...
    if (session[:hmm_user].nil?)
      ...
      if @contest_vote <= 0
        ...
        if @vote.save
          ...
          if(moment_type=="video")
            redirect_to :controller => "familymemory_contests", :action => 'manage_videomoment_vote',:id => params[:id]
          else
            redirect_to :controller => "familymemory_contests", :action => 'manage_moments_vote',:id => params[:id]
          end
        else
          $notice_vote
          flash[:notice_vote] = 'Your was not submitted due to technical issue... Please try again!!'
  ...
  def contest_sub_chapter
    ...
    if (params[:tagtype]=='chapter' || params[:tagtype]=='mobile_uploads')
      ...
      @subchaps = SubChapter.paginate :per_page => items_per_page,:page => params[:page], :order => sort, :select => "a.id, a.v_image, a.img_url, a.sub_chapname, a.d_updated_on, b.id as user_content_id", :joins => "as a, user_contents as b", :conditions => "a.id = b.sub_chapid and b.e_filetype = '#{session[:contests_type]}' and a.tagid=#{params[:tagid]} and a.status='active' ", :group => "a.id" # ,:order => " #{@sort.to_s} #{@order.to_s} "
      ...
      #  end
      @subchapter = SubChapter.find(:all, :select => "a.*, b.id as user_content_id", :joins => "as a, user_contents as b", :conditions => "a.id = b.sub_chapid and b.e_filetype = '#{session[:contests_type]}' and a.tagid=#{params[:tagid]} and a.status='active' ")
      ...
      if(countcheck==1)
        ...
        redirect_to :controller => "familymemory_contests" , :action => 'contest_gallery', :id => @hmmuser[0]['family_name'], :subchapid => @subchapter[0]['id']
      ...
      if(@content_count >0)
        ...
        redirect_to :controller=>'familymemory_contests' , :action=>'contest_moments' ,:id =>params[:id],:moment_id => @gallery_id
      else
        flash[:notice] = 'You have no moments'
        @tag_id = params[:tagid]
        redirect_to :controller=>'familymemory_contests' , :action=>'no_moments' ,:id =>params[:id],:moment_id => 0
      end
    end
  ...
  def contest_gallery
    ...
    @sub_chapters_gallerie = Galleries.paginate :per_page => items_per_page,:page => params[:page], :order => sort,
      :select => "a.id, a.v_gallery_image, a.img_url, a.v_gallery_name, a.d_gallery_date, b.id as user_content_id", :joins => "as a, user_contents as b",
    ...
    puts countcheck
    if(countcheck==1)
      redirect_to :controller => "familymemory_contests" , :action => 'contest_moments', :id => @hmmuser[0]['family_name'], :moment_id => @sub_chapters_gallerie[0]['id']
    else
      render :layout => true
    end
  ...
  def select_contestmoment
    ...
    contest_phase=params[:phase]
    contets_details=ContestDetail.find(:first,:conditions=>"contest_phase='#{contest_phase}'")
    ...
    if(issubmitted.length == 0)
      ...
      if (@contest_namecheck.length == 0)
        ...
        if params[:type]=="image"
          title="#{contets_details.name} Photo Contest"
        elsif params[:type]=="video"
          title="#{contets_details.name} Video Contest"
        end
        @contest = Contest.new
        @contest['uid']=user.id
        @contest['moment_id']=params[:momentid]
        ...
        flash[:result_entry] = "Your Entry has been submitted. After Your entry has been approved you will receive an email notification so that you may vote your contestant, and inivite your friends to vote for your family"
      else
        @result = "false"
        flash[:result_entry] = "Contestant name already present!!"
      end
    else
      @result = "submited"
      flash[:result_entry] = "Already submitted this moment!!"
    end
    redirect_to "/familymemory_contests/submit_entry/#{params[:id]}?moment_id=#{params[:momentid]}"
  end
