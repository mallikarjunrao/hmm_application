class FriendsFamilyController < ApplicationController
	...
	def getFriendsList
		...
		@nonHmmFriends = NonhmmUser.find(:all, :conditions => "uid=#{logged_in_hmm_user.id}",:group=>"v_email")
  ...
  uid=logged_in_hmm_user.id
     ...
     if (params[:query_hmm_user].nil?)   
        ...
        @hmm_users = HmmUser.paginate  :select => "id as fid, v_myimage, v_fname",  :order => sort, :per_page => items_per_page,
        :conditions => conditions12,:page => params[:page]
     ...
     if( params[:query]=="" && params[:query1]=="" && params[:query_hmm_user] =="" )
        ...
        @hmm_users = HmmUser.paginate :hmm_users ,:joins=>"as b , family_friends  as a ", :order => sort, :per_page => items_per_page,
        :conditions => "a.uid=#{uid} && b.id=a.fid && a.status='accepted' and a.block_status='unblock'", :page => params[:page]
     else
        flag=1
...
#        :conditions => conditions
        ...
        :conditions => conditions1, :page => params[:page]
     end
     end
  ...
  def feed_update
    ...
    if( params[:id] == '1' )
    link='http://fusion.google.com/add?source=atgs&feedurl=http://blog.holdmymemories.com/?feed=rss2'
    end
    
    if( params[:id] == '2')
    link='http://add.my.yahoo.com/rss?url=http://blog.holdmymemories.com/?feed=rss2'
    end
    
    if ( params[:id] == '3' )
    link='http://blog.holdmymemories.com/?feed=rss2'
    end
    
    if ( params[:id] == '4' )
    link='http://feeds.feedburner.com/HoldmymemoriesBlog'
    end
    
     if ( params[:id] == '5' )
    link='http://www.pageflakes.com/subscribe.aspx?url=http://blog.holdmymemories.com/?feed=rss2'
    end
    
    @content = FeedUpdate.find(params[:id])
