class EvelethsfamilyController < ApplicationController
  ...
  def chapters_display
    uid = 4;
    @results = Tag.find(:all, :conditions => "uid=#{uid} and status='active' and default_tag='yes' " , :order => "order_num ASC" )
  ...
  def coverFlowImages
     	 @subchapters = SubChapter.find(:all, :conditions=>"tagid=#{params[:id]} and status='active' ", :order => "order_num ASC")
	 @chapterid = params[:id]	 
	  render :layout => false
	end
  def biographies
  ...
  def videoCoverflow
    ...
    @contents = UserContent.find(:all, :conditions => "gallery_id=#{params[:id]} and status='active' #{@e_acess} ", :order => "order_num ASC")
    render :layout => false
  end
  ...
  def showGallery
    ...
    @contents = UserContent.find(:all, :group => 'v_filename', :conditions => "gallery_id=#{params[:id]} and status='active' #{@e_acess}", :order => "order_num ASC")
    render :layout => false
  end
