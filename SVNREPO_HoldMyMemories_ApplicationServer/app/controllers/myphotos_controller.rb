...
before_filter :authenticate, :except => [:addallphoto_from_wpf]
...
def create
 ...
 def edit
  ...
  def addPhoto
    ...
    @val['img_url'] = @content_path['content_path']
  ...
  def addallalbumphoto
    files = params[:photolist]
    logger.info("photolist : - "+params[:photolist])
    subchapter=SubChapter.find(:all, :conditions => "tagid='#{params[:tagid]}'")
    subchapid=subchapter[0].id
    gallery=Galleries.find(:all,:conditions => "subchapter_id='#{subchapid}'")
    galid=gallery[0].id
    @idstring = "";
    ...
    for file in filesarray
      ...
      @val['e_filetype'] = gallery[0].e_gallery_type #file['e_filetype'];#params[:e_filetype]
      logger.info("File type :- "+gallery[0].e_gallery_type)
      filename = filedata[0];
      logger.info("filename :- "+filename)
      splitresult= filename.split('.');#params[:v_filename].split('.')
      @val['v_tagphoto'] =splitresult[0]
      @val['sub_chapid'] = subchapid #file['sub_chapid']; #params[:sub_chapid]
      @val['uid'] = params[:userId]
      @val['e_access'] = params[:permission]#'semiprivate'
      @val['tagid'] = params[:tagid]
      @val['d_createddate'] = Time.now
      @val['d_momentdate'] = Time.parse(filedata[1])
      @val['gallery_id'] = galid
      @val['v_desc'] = 'Enter description here.'
      @val.save
      @results =  Tag.find(:all, :conditions=> "default_tag='no' and uid=#{logged_in_hmm_user.id}")
      for res in @results
 ...
 if( params[:galleryid]!=nil)
  ...
  if(params[:Id])
   ...
   if(session[:share_with_hmm]!=nil)
    ...
    if(flag==1)
      @user_content = UserContent.paginate  :conditions => " id in (#{idarr})  " ,:order =>'id asc', :per_page => 1, :page => params[:page]
      if(params[:page]==nil)
 ...
 if(session[:share]=="check" )
    ...
    if(params[:page]==nil)
      ...
      for check in @allrecords
       ...
       @user_content = UserContent.paginate :conditions => " id in (#{idarr}) ", :per_page => 1, :page => params[:page]
     end
     @norecords=""
   else
    if(params[:page]==nil)
     ...
     @user_content = UserContent.paginate :conditions => "gallery_id='#{@usercontent.gallery_id}' and status='active'  #{eacess} ",:order =>'id desc', :per_page => 1, :page => params[:page]
   end
   @norecords=""
  end
end
...
#     @img = @image.split("&")
 ...
 def sharemomentWorker
    ...
    if params[:user]
      @HmmUser = params[:user]
      hmmids = @HmmUser.join(",")
      @HmmUser = HmmUser.find(:all, :conditions => "id in (#{hmmids})") 
      for i in @HmmUser
    ...
    if params[:nhmmuser]
      @nonHmmUser = params[:nhmmuser]
      nonhmmids = @nonHmmUser.join(",")
    ...
    if params[:email]
     ...
     if(share_max_id == '')
      share_max_id = '0'
        ...
        @check_HmmUser = HmmUser.count(:all, :conditions => "v_e_mail='#{k}'") 
        if(@check_HmmUser > 0)
          @check_HmmUser = HmmUser.find(:all, :conditions => "v_e_mail='#{k}'") 
          @blocked_friend = FamilyFriend.count(:all,:conditions => "uid='#{@check_HmmUser[0]['id']}' and fid='#{logged_in_hmm_user.id}' and block_status='block'")
          if(@blocked_friend <= 0)
            @blocked_friend =  0
          else
            @blocked_friend = 1
  ...
  def addPhotoalbum
    ...
    splitresult=params[:v_filename].split('.')
    @val['v_tagphoto'] =splitresult[0]
    @val['sub_chapid'] = subchapid
    @val['uid'] = params[:userId]
    @val['e_access'] = permission #'semiprivate'
    @val['tagid'] = params[:tagid]
    @val['d_createddate'] = Time.now
    @val['img_url'] = @content_path['content_path']
    @val['d_momentdate'] = Time.parse(params[:createddate])
