class SeniorscontestController < ApplicationController
 layout "application"
  ...
  def hmm_contest
    @topentries_photo_0_to_2 = Contest.find(:all,:select => "a.*, b.*, a.id as contest_id, a.first_name as contest_fname" ,:joins=>"as a, user_contents as b",
      :conditions => "a.moment_id = b.id and a.moment_type = 'image' and a.contest_phase='#{seniorcontest_phase}' and a.status='active'", :order => "a.votes desc", :limit =>"8")
...
#      :conditions => "a.moment_id = b.id and a.moment_type = 'image' and a.contest_phase='#{seniorcontest_phase}'and a.age_groups='26-35' and a.status='active'", :order => "a.votes desc", :limit =>"2")
#
...
#      :conditions => "a.moment_id = b.id and a.moment_type = 'image' and a.contest_phase='#{seniorcontest_phase}'and a.age_groups='36-50' and a.status='active'", :order => "a.votes desc", :limit =>"2")
#
...
#      :conditions => "a.moment_id = b.id and a.moment_type = 'image' and a.contest_phase='#{seniorcontest_phase}'and a.age_groups='50+' and a.status='active'", :order => "a.votes desc", :limit =>"2")
#
  ...
  def authenticate
    ...
    if is_userlogged_in?
      if logged_in_hmm_user.e_user_status == 'unblocked'
        ...
        redirect_to :controller => 'familywebsite_seniorscontest', :action =>'contest_chapter', :id => logged_in_hmm_user.family_name
      else
        reset_session
    ...
    if @authenticate_unid == 1
      ...
      if is_userlogged_in?
        if logged_in_hmm_user.e_user_status == 'unblocked'
          ...
          if params[:contest_phase]
            redirect_to :controller => 'seniorscontest', :action =>'contest_upload',:id=>params[:contest_phase]
          else
            redirect_to :controller => 'seniorscontest', :action =>'contest_upload'
          end
        end
...
#  end
  ...
  def select_contestmoment
    issubmitted = Contest.find_by_sql("select *  from contests where contests.moment_id='#{params[:momentid]}' and contests.uid=#{logged_in_hmm_user.id} and contest_phase = '#{seniorcontest_phase}'")
    ...
    if(issubmitted.length == 0)
      @contest_namecheck = Contest.find_by_sql("select *  from contests where contests.first_name='#{params[:fname]}' and contests.uid='#{logged_in_hmm_user.id}' and moment_type = '#{params[:type]}' and contest_phase = '#{seniorcontest_phase}' ")
  ...
  def moments_vote
    ...
    @contest_memoriable = Contest.find(:all, :conditions => "moment_type = 'image' and contest_phase='#{seniorcontest_phase}'")
    if (params[:query].nil?)
      conditions = "a.moment_id = b.id and a.moment_type = 'image' and a.status = 'active' and a.contest_phase='#{seniorcontest_phase}'"
    else
      params[:query] = (params[:query].gsub("'","")).gsub('"',"")
      conditions = "a.moment_id = b.id and a.moment_type = 'image' and a.status = 'active' and ( a.first_name like '#{params[:query]}%') and a.contest_phase='#{seniorcontest_phase}' "
    end
  ...
  def videomoment_vote
    ...
    @contest_memoriable = Contest.find(:all, :conditions => "moment_type='video' and contest_phase='#{seniorcontest_phase}'")
    if (params[:query].nil?)
      conditions = "a.moment_id = b.id  and  a.moment_type = 'video' and a.status='active' and a.contest_phase='#{seniorcontest_phase}'"
    else
      params[:query] = (params[:query].gsub("'","")).gsub('"',"")
      conditions = "a.moment_id = b.id   and  a.moment_type = 'video' and a.status='active' and ( a.first_name like '#{params[:query]}%') and a.contest_phase='#{seniorcontest_phase}'"
  ...
  def momentdetails
    ...
    if(params[:page]==nil || params[:page]=='' || params[:hmm] ==nil)
      @counts =  Contest.count(:all, :conditions => " moment_type='image' and status='active' and contest_phase='#{seniorcontest_phase}' ",:order =>'contest_entry_date  desc')
      @allrecords =  Contest.find(:all, :conditions => " moment_type='image' and status='active' and contest_phase='#{seniorcontest_phase}'" ,:order =>'contest_entry_date  desc')
      page=0
  ...
  def videomoment_details
    @moment_details = UserContent.find(:all, :conditions => "id='#{params[:id]}'")
    @contests_fname = Contest.find(:all, :conditions => "moment_id='#{params[:id]}' and contest_phase='#{seniorcontest_phase}'")
    #@journal = JournalsPhoto.find(:all, :conditions => "user_content_id = #{params[:id]} ")
    @journal = Blog.find(:all, :conditions => "blog_type_id = #{params[:id]} and user_id=#{@moment_details[0].uid}")
    @contest_image="#{@moment_details[0].img_url}/user_content/videos/thumbnails/#{@moment_details[0].v_filename}.jpg"
    entrants_name=@contests_fname[0].first_name
    @contest_description =  "Hello Friends!\n\nI have entered in HoldMyMemories.com All Glammed Up Video Contest. Please help #{entrants_name} to win by clicking on the above link and casting your vote!"
    @contest_title=" All Glammed Up Video Contest. Brought to you by HoldMyMemories.com."
  ...
  def vote
    ...
    @contests_fname = Contest.find(:all, :conditions => "moment_id='#{@contests_det.moment_id}' and contest_phase='#{seniorcontest_phase}'")
  ...
  def create_vote
    ...
    if @contest_vote <= 0
      ...
      if @vote.save
        ...
        Postoffice.deliver_kidsvoteconform(params[:email],contetsid, unid,moment_filename,contetsfname,user_fname,user_lname,moment_type,paths.proxyname,momentid,moment_url,seniorcontest_phase)
        if(moment_type=="video")
          redirect_to :controller => "seniorscontest", :action => 'videomoment_vote'
        else
          redirect_to :controller => "seniorscontest", :action => 'moments_vote'
        end
      else
        $notice_vote
  ...
  def share_via_email_sub
    ...
    $notice_vote1
    flash[:notice_vote] = 'Thank-you for casting your vote.  You will shortly receive an email verification that you must accept for your vote to be counted.  This helps us make sure that all votes are valid votes.  Your email address will not be shared with any other company.'
    Postoffice.deliver_shareviaemailkids(params[:email],contetsid,unid,moment_filename,contetsfname,user_fname,user_lname,moment_type,params[:view],paths.proxyname,momentid,moment_url, seniorcontest_phase)
